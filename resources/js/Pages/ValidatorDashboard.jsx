...
export default function ValidatorDashboard() {
  ...
  async function handleExportBarangay() {
    try {
      const scope = showTable === 'survey' ? 'survey' : showTable === 'submitted' ? 'submitted' : ''
      if (!scope) return
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 h-screen overflow-y-auto p-6 bg-gray-50">
        ...
        <section className={`${(showTable !== 'survey' && showTable !== 'submitted') && 'hidden'} mt-6`}>
                  ...
                  <tbody>
                    {modalRows.map((r, idx) => (
                      <tr key={idx} className="even:bg-gray-50">
                        ...
                        <td className="p-2">{String(r.barangay || '').replace(/_/g,' ')}</td>
